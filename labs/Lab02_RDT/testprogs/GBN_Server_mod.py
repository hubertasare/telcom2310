...
def get_args(argv):
    parser = argparse.ArgumentParser(
        description="UDP File Transfer Server using Go-Back-N protocol")
    parser.add_argument('-p', '--port', required=False,
                        default=12000, type=int)
    parser.add_argument('-f', '--filename', required=False,
                        default="copied_file.txt")
...
def main(argv):
    ...
    while True:
        try:
            ...
            # set save as name as the packet of type filename
            if pkt_header.type == PacketHeader.TYPE_FILENAME and outfile is None:
                outputfilename = data.decode()
